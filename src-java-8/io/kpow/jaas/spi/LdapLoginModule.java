...
//  ========================================================================
//
// 2022-02-01: Modified by Derek Troy-West (derek@operatr.io) to support bindPassword in OBF or AES format
//
// See original source: org.eclipse.jetty.jaas.spi.LdapLoginModule
//

package io.kpow.jaas.spi;
...
public class LdapLoginModule extends AbstractLoginModule
{
                           ...
                           Map<String, ?> options)
    {
        ...
        // DTW modifications to support OBF: or AES: format of bindPassword
        String bindPasswordIn = (String) options.get("bindPassword");
        String bindPasswordPlain;

        try {
            if (bindPasswordIn != null && bindPasswordIn.startsWith(Password.__OBFUSCATE)) {
                Password password = new Password(bindPasswordIn);
                bindPasswordPlain = password.toString();
            } else if (bindPasswordIn != null && bindPasswordIn.startsWith("AES:")) {
                String payload = bindPasswordIn.substring(4);
                bindPasswordPlain = Decoder.text(payload);
            } else {
                bindPasswordPlain = bindPasswordIn;
            }
        } catch (Exception ex) {
            LOG.info("bindPassword error", ex);
            bindPasswordPlain = bindPasswordIn;
        }

        // DTW modifications to support OBF: or AES: format of trustStorePassword
        String trustStorePassword = System.getProperty("javax.net.ssl.trustStorePassword");

        try {
            if (trustStorePassword != null && trustStorePassword.startsWith(Password.__OBFUSCATE)) {
                Password password = new Password(bindPasswordIn);
                System.setProperty("javax.net.ssl.trustStorePassword", password.toString());
            } else if (trustStorePassword != null && trustStorePassword.startsWith("AES:")) {
                String payload = trustStorePassword.substring(4);
                String password = Decoder.text(payload);
                System.setProperty("javax.net.ssl.trustStorePassword", password);
            }
        } catch (Exception ex) {
            LOG.info("trustStorePassword error", ex);
        }

        _bindPassword = bindPasswordPlain;
